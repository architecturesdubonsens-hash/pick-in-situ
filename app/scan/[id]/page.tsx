...
export default function ScanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const data = MOCK_DATA[id] ?? MOCK_DATA["demo-appartement"];
          ...
          <div className="flex rounded-lg border border-slate-200 overflow-hidden">
            {(["3d", "plan"] as const).map((v) => (
              <button
                ...
                style={view === v ? { background: "var(--navy)" } : {}}
              >
                {v === "3d" ? "3D" : "Plan"}
              </button>
            ))}
          </div>
          <button className="px-3 py-1.5 text-sm border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600">↓ DXF</button>
          <button className="px-3 py-1.5 text-sm border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600">↓ PDF</button>
          <button className="px-3 py-1.5 text-sm rounded-lg text-white" style={{ background: "var(--orange)" }}>↓ glTF</button>
        </div>
      </div>
        ...
        <aside className="w-72 bg-white border-l border-slate-200 flex flex-col overflow-y-auto">
          ...
          <div className="p-4 border-b border-slate-100">
            ...
            {[
              ...
              { icon: "▭", label: "Fenêtres", count: 6, color: "#93c5fd" },
          ...
          <div className="p-4">
            ...
            <div className="grid gap-2">
              {[
                { icon: "📐", label: "DXF pour ArchiCAD" },
                { icon: "📄", label: "Plan PDF coté" },
                { icon: "🧊", label: "Modèle glTF / OBJ" },
                { icon: "🏗️", label: "IFC (BIM)" },
              ].map(({ icon, label }) => (
                <button
                  key={label}
                  className="w-full text-left px-3 py-2.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-sm text-slate-700 flex items-center gap-2"
                >
